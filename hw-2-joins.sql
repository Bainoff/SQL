...
join roles_salary on employee_salary.salary_id = roles_salary.id_salary 
...
join roles_salary on salary.id = roles_salary.id_salary
join roles on roles_salary.id_role = roles.id
where roles.role_name = '%Junior%QA engineer';
...
select count(employees.id) as "middle counter"
from employees
join roles_employee on employees.id = roles_employee.employee_id
join roles on roles_employee.role_id = roles.id
where roles.role_name like '%Middle%';
